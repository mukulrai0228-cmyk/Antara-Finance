...
export const FinanceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchUserData = async (authUser: any) => {
    ...
    try {
      ...
      let savedLoans = localStorage.getItem(`spendly_loans_emis_${userId}`);
      if (!savedLoans) {
        savedLoans = localStorage.getItem(`antara_loans_emis_${userId}`);
        if (savedLoans) {
          localStorage.setItem(`spendly_loans_emis_${userId}`, savedLoans);
          localStorage.removeItem(`antara_loans_emis_${userId}`);
        }
      }
      if (savedLoans) {
        try {
      ...
      localStorage.setItem(`spendly_loans_emis_${userId}`, JSON.stringify(migratedLoans));
    } catch (err) {
  ...
  const resetAllData = async () => {
    ...
    try {
      ...
      localStorage.removeItem(`spendly_loans_emis_${authUserId}`);
  ...
  const deleteVehicle = async (id: string) => {
    try {
      ...
      if (authUserId) {
        ...
        localStorage.setItem(`spendly_loans_emis_${authUserId}`, JSON.stringify(updatedLoans));
      }
    } catch (err) {
    ...
    const runSync = async () => {
      ...
      try {
        ...
        if (hasUpdates) {
          ...
          localStorage.setItem(`spendly_loans_emis_${authUserId}`, JSON.stringify(cleanedLoans));
          return;
        }
