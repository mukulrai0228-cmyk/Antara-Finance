...
export const ExpensesView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Controls: Horizontal Month Selector & View Mode Switcher */}
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white border border-slate-200/85 p-4 rounded-3xl shadow-sm">
        
        {/* Month Selector Carousel */}
        <div className="flex gap-1 overflow-x-auto pb-1 sm:pb-0 scrollbar-none max-w-full sm:max-w-md">
          {MONTHS.map((m) => {
        ...
        {/* View togglers */}
        <div className="flex items-center gap-2 border border-slate-150 p-1 rounded-2xl bg-slate-50/50">
          <button
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-5 shadow-sm animate-fade-in">
          ...
          <div className="grid grid-cols-7 gap-2">
            ...
            {Array.from({ length: startDayOffset }).map((_, i) => (
              <div key={`offset-${i}`} className="bg-slate-50/30 border border-transparent rounded-2xl aspect-square" />
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, idx) => {
              ...
              return (
                <button
                  ...
                  className={`aspect-square p-2 border rounded-2xl text-left flex flex-col justify-between hover:border-blue-300 hover:bg-slate-50/50 transition-all ${
                    isTodayDay
                      ? 'border-blue-600 bg-blue-50/20'
                      : 'border-slate-100 bg-white'
                  }`}
                >
                  <span className={`text-xs font-bold ${isTodayDay ? 'text-blue-600' : 'text-slate-505'}`}>
                    {dayNum}
                  </span>
                  
                  {spend > 0 ? (
                    <div className="w-full text-right leading-none mt-1">
                      <span className="text-[9px] font-extrabold text-slate-800">
                        ₹{spend >= 1000 ? `${(spend / 1000).toFixed(1)}k` : spend}
                      </span>
                      <div className="w-1.5 h-1.5 rounded-full bg-blue-600 ml-auto mt-0.5" />
                    </div>
                  ) : (
                    <div className="w-full text-right opacity-0 group-hover:opacity-100">
                    ...
                    <tr key={tx.id} className="hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="p-4 pr-6 text-center">
                        <button
                          ...
                          className="p-1.5 text-slate-400 hover:text-red-650 rounded-lg hover:bg-red-50/50 transition-all"
